...
function modelBox(root: THREE.Object3D): THREE.Box3 | null {
...
function cycleView(runtime: Runtime): void {
  if (!runtime.build) return;
  const box = modelBox(runtime.build.root);
  if (!box) return;
  ...
  runtime.viewIndex = (runtime.viewIndex + 1) % views.length;
  const offset = views[runtime.viewIndex];
...
function renderTransformPreview(runtime: Runtime): void {
  ...
  transient.name = `linkoteq-${interaction.operation ?? "transform"}-preview`;
...
export default function ThatOpenViewportV05({
  model,
  selection,
  onSelect,
}: Props) {
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      setCopyPreview(resolve(event));
      const current = runtimeRef.current;
      if (current) renderTransformPreview(current);
    };
    ...
    const onViewCycle = () => {
      const current = runtimeRef.current;
      if (current) cycleView(current);
    };

    const onViewSelect = () => {
      const current = runtimeRef.current;
      if (current) current.camera.controls.enabled = true;
    };
    ...
    window.addEventListener("linkoteq:view-cycle", onViewCycle);
    window.addEventListener("linkoteq:view-select", onViewSelect);
    ...
    return () => {
      ...
      window.removeEventListener("linkoteq:view-cycle", onViewCycle);
      window.removeEventListener("linkoteq:view-select", onViewSelect);
    ...
    >
      ...
      >
        {interaction.mode === "select"
          ? "That Open · Select / Orbit / Pan / Zoom"
          : interaction.message}
