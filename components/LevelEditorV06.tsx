...
function LevelRow({ level, model, onModelChange }: { level: Level; model: StructuralModel; onModelChange: Props["onModelChange"] }) {
  ...
  const save = () => {
    try {
      const result = updateLevel(model, level.id, { name, elevation: Number(elevation) });
      onModelChange(result.model, `Level ${result.level.id} updated.`);
    } catch (error) {
      onModelChange(model, error instanceof Error ? error.message : "Level update failed.");
    }
  };
  ...
  const remove = () => {
    try {
      ...
      onModelChange(model, error instanceof Error ? error.message : "Level removal failed.");
    }
  };
...
export default function LevelEditorV06({ model, onModelChange }: Props) {
    ...
    <div className="lgModalBackdrop" onMouseDown={() => setOpen(false)}>
      <section className="panelBlock lgPanel lgPortalPanel" onMouseDown={(event) => event.stopPropagation()}>
        <header className="lgHeader">
          <div><span>MODEL SETUP</span><h3>Levels</h3><p>Add, edit, and remove canonical elevation references.</p></div>
