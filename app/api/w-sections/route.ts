  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Unable to load CISC W-section dataset." },
        { status: 502 },
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(
      `${DESIGN_API_BASE_URL.replace(/\/$/, "")}/api/v1/calculations/w-section/core`,
      {
        method: "POST",
    ...
    const body = await response
      .json()
      .catch(() => ({ error: "Invalid response from W-section design service." }));
