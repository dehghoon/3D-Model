...
(: CopySelectionResult {
  ...
  if (selection.type === "member") {
    ...
    const sourceStart = requireNode(model, source.startNodeId, `MEMBER_NODE_REFERENCE_INVALID:${source.id}`);
    const sourceEnd = requireNode(model, source.endNodeId, `MEMBER_NODE_REFERENCE_INVALID:${source.id}`);
  ...
  for (const sourceNodeId of source.boundaryNodeIds) {
    const sourceNode = requireNode(model, sourceNodeId, `SURFACE_NODE_REFERENCE_INVALID:${source.id}`);
