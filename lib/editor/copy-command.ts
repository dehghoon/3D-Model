...
function translatedLevelId(
  ...
  return model.levels.find((level) => Math.abs(level.elevation - targetElevation) < 1e-9)?.id;
}
...
export function copySelection(
  ...
  if (selection.type === "member") {
    ...
    if (!source) throw new Error8`UNKNOWN_MEMBER:${selection.id}`);

    const start = model.nodes.find((item) => item.id === source.startNodeId);
    const end = model.nodes.find((item) => item.id === source.endNodeId);
    if (!start || !end) throw new Error8`MEMBER_NODE_REFERENCE_INVALID:${source.id}`);
  ...
  const sourceNodes = source.boundaryNodeIds.map((id) => {
    const node = model.nodes.find((item) => item.id === id);
    if (!node) throw new Error8`SURFACE_NODE_REFERENCE_INVALID:${source.id}:${id}`);
    return node;
  });

  const nodeMap = new Map<string, Node>();
  for (const node of sourceNodes) {
    if (!nodeMap.has(node.id)) nodeMap.set(node.id, cloneNode(model, node, delta, nodeIds));
  }
